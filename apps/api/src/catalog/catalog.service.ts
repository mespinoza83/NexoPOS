...
export class CatalogService {
  ...
  async inventoryReport(user: AuthenticatedUser, query: Record<string, string | undefined>) {
    ...
    const products = await this.prisma.product.findMany({ where: { businessId: user.businessId, ...(query.categoryId ? { categoryId: query.categoryId } : {}), ...(search ? { OR: [{ name: { contains: search, mode: 'insensitive' } }, { internalCode: { contains: search, mode: 'insensitive' } }, { barcode: { contains: search, mode: 'insensitive' } }] } : {}) }, select: { id: true, internalCode: true, barcode: true, name: true, status: true, purchasePrice: true, inventoryUnit: true, presentation: true, category: { select: { id: true, name: true } }, inventory: { where: { branchId }, select: { quantity: true, minimumQuantity: true } }, inventoryMovements: { where: { branchId }, select: { createdAt: true }, orderBy: { createdAt: 'desc' }, take: 1 } }, orderBy: { name: 'asc' } });
    const rows = products.map((product) => { const quantity = Number(product.inventory[0]?.quantity ?? 0); const minimumQuantity = Number(product.inventory[0]?.minimumQuantity ?? 0); const unitCost = Number(product.purchasePrice); const stockLevel = quantity <= 0 ? 'OUT_OF_STOCK' : minimumQuantity <= 0 ? 'HEALTHY' : quantity < minimumQuantity ? 'BELOW_MINIMUM' : quantity === minimumQuantity ? 'AT_MINIMUM' : quantity <= minimumQuantity * 1.2 ? 'NEAR_MINIMUM' : 'HEALTHY'; return { id: product.id, internalCode: product.internalCode, barcode: product.barcode, name: product.name, productStatus: product.status, inventoryUnit: product.inventoryUnit, presentation: product.presentation, category: product.category, quantity, minimumQuantity, unitCost, inventoryValue: Math.round(quantity * unitCost * 100) / 100, stockLevel, lastMovementAt: product.inventoryMovements[0]?.createdAt ?? null }; });
    const filtered = query.stockLevel ? rows.filter((row) => row.stockLevel === query.stockLevel) : rows;
    const [categories, business] = await Promise.all([this.prisma.category.findMany({ where: { businessId: user.businessId, active: true }, select: { id: true, name: true }, orderBy: { name: 'asc' } }), this.prisma.business.findUniqueOrThrow({ where: { id: user.businessId }, select: { legalName: true, commercialName: true, taxId: true, address: true, phone: true, defaultCurrency: true, timezone: true, receiptMessage: true, receiptPaperWidth: true } })]);
    return { products: filtered, categories, business, totals: { products: filtered.length, units: filtered.reduce((sum, row) => sum + row.quantity, 0), value: Math.round(filtered.reduce((sum, row) => sum + row.inventoryValue, 0) * 100) / 100, alerts: filtered.filter((row) => row.stockLevel !== 'HEALTHY').length } };
  }
  ...
  async updateProduct(user: AuthenticatedUser, productId: string, dto: UpdateProductDto) {
    ...
    try {
      const product = await this.prisma.product.update({ where: { id: productId }, data: {
        ...
        ...(dto.inventoryUnit !== undefined ? { inventoryUnit: dto.inventoryUnit } : {}), ...(dto.saleUnit !== undefined ? { saleUnit: dto.saleUnit } : {}),
        ...(dto.saleUnitFactor !== undefined ? { saleUnitFactor: dto.saleUnitFactor } : {}), ...(dto.allowFractionalSale !== undefined ? { allowFractionalSale: dto.allowFractionalSale } : {}),
