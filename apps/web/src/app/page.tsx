...
export default function HomePage() {
  ...
  if (user) {
    if (activeView === 'admin') return adminData ? <AdminView user={user} data={adminData} loading={adminLoading} onBack={() => setActiveView('dashboard')} onSales={openSales} onInventory={openCatalog} onCash={openCash} onRefresh={openAdmin} onLogout={handleLogout} /> : <AdminStatusView loading={adminLoading} error={adminError} onRetry={openAdmin} onBack={() => setActiveView('dashboard')} />;
    if (activeView === 'inventory') {
      return <InventoryView user={user} products={products} categories={categories} movements={inventoryMovements} entries={inventoryEntries} counts={inventoryCounts} loading={catalogLoading} onRefresh={openCatalog} onBack={() => setActiveView('dashboard')} onSales={openSales} onCash={openCash} onAdmin={openAdmin} onLogout={handleLogout} />;
    }
    if (activeView === 'sales' && salesSetup) return <SalesView user={user} products={products} setup={salesSetup} invoices={salesHistory} loading={salesLoading} onRefresh={openSales} onBack={() => setActiveView('dashboard')} onInventory={openCatalog} onCash={openCash} onAdmin={openAdmin} onLogout={handleLogout} />;
    if (activeView === 'cash' && cashData) return <CashView user={user} data={cashData} loading={cashLoading} onRefresh={openCash} onBack={() => setActiveView('dashboard')} onSales={openSales} onInventory={openCatalog} onAdmin={openAdmin} onLogout={handleLogout} />;
...
function CashView({ user, data, loading, onRefresh, onBack, onSales, onInventory, onAdmin, onLogout }: { user: AuthUser; data: CashData; loading: boolean; onRefresh: () => Promise<void>; onBack: () => void; onSales: () => Promise<void>; onInventory: () => Promise<void>; onAdmin: () => Promise<void>; onLogout: () => void }) {
...
function SalesView({ user, products, setup, invoices, loading, onRefresh, onBack, onInventory, onCash, onAdmin, onLogout }: { user: AuthUser; products: CatalogProduct[]; setup: SalesSetup; invoices: SalesInvoice[]; loading: boolean; onRefresh: () => Promise<void>; onBack: () => void; onInventory: () => Promise<void>; onCash: () => Promise<void>; onAdmin: () => Promise<void>; onLogout: () => void }) {
  ...
  async function printInvoice(invoice: SalesInvoice) {
    ...
    if (!response.ok) { const result = await response.json().catch(() => ({})) as { message?: string }; await Swal.fire({ icon: 'error', title: 'No se puede imprimir', text: result.message ?? 'No tiene permiso para reimprimir.' }); return; }
    window.print();
  }
...
function AdminView({ user, data, loading, onBack, onSales, onInventory, onCash, onRefresh, onLogout }: { user: AuthUser; data: AdminData; loading: boolean; onBack: () => void; onSales: () => Promise<void>; onInventory: () => Promise<void>; onCash: () => Promise<void>; onRefresh: () => Promise<void>; onLogout: () => void }) {
