...
type BusinessSettings = { legalName: string; commercialName?: string | null; taxId?: string | null; address?: string | null; phone?: string | null; email?: string | null; logoUrl?: string | null; defaultCurrency: string; timezone: string; receiptPaperWidth: number; receiptMessage?: string | null; ivaRate?: string | number; taxesEnabled?: boolean };
...
function printProductsPdf(data: ProductReportData, filters: { search: string; dateFrom: string; dateTo: string }) { const filterText = [filters.search && `Producto: ${filters.search}`, filters.dateFrom && `Desde: ${filters.dateFrom}`, filters.dateTo && `Hasta: ${filters.dateTo}`].filter(Boolean).join(' | '); printReportPdf('Productos vendidos y rentabilidad', data.business, filterText, [['Unidades netas', data.totals.netQuantity.toFixed(2)], ['Venta sin impuesto', `C$ ${data.totals.netSales.toFixed(2)}`], ['Costo', `C$ ${data.totals.cost.toFixed(2)}`], ['Utilidad', `C$ ${data.totals.estimatedProfit.toFixed(2)}`]], ['Código', 'Producto', 'Categoría', 'Vendidas', 'Devueltas', 'Neto', 'Venta', 'Costo', 'Utilidad'], data.products.map((product) => [product.internalCode, product.name, product.category, product.soldQuantity.toFixed(2), product.returnedQuantity.toFixed(2), product.netQuantity.toFixed(2), `C$ ${product.netSales.toFixed(2)}`, `C$ ${product.cost.toFixed(2)}`, `C$ ${product.estimatedProfit.toFixed(2)}`])); }
function printInventoryPdf(data: InventoryReportData, filters: { search: string; categoryId: string; stockLevel: string }) { const filterText = [filters.search && `Producto: ${filters.search}`, filters.categoryId && `Categoría: ${data.categories.find((item) => item.id === filters.categoryId)?.name}`, filters.stockLevel && `Nivel: ${filters.stockLevel}`].filter(Boolean).join(' | '); printReportPdf('Inventario valorizado', data.business, filterText, [['Productos', String(data.totals.products)], ['Unidades', data.totals.units.toFixed(2)], ['Valor', `C$ ${data.totals.value.toFixed(2)}`], ['Alertas', String(data.totals.alerts)]], ['Código', 'Producto', 'Categoría', 'Existencia', 'Mínimo', 'Nivel', 'Costo', 'Valor'], data.products.map((product) => [product.internalCode, product.name, product.category.name, product.quantity.toFixed(2), product.minimumQuantity.toFixed(2), getStockAlert(product.quantity, product.minimumQuantity).label, `C$ ${product.unitCost.toFixed(2)}`, `C$ ${product.inventoryValue.toFixed(2)}`])); }
function printCashPdf(data: CashReportData, filters: { dateFrom: string; dateTo: string; status: string }) { const filterText = [filters.dateFrom && `Desde: ${filters.dateFrom}`, filters.dateTo && `Hasta: ${filters.dateTo}`, filters.status && `Estado: ${filters.status}`].filter(Boolean).join(' | '); printReportPdf('Caja y cierres', data.business, filterText, [['Turnos', String(data.totals.sessions)], ['Ventas', `C$ ${data.totals.salesTotal.toFixed(2)}`], ['Efectivo', `C$ ${data.totals.cashSales.toFixed(2)}`], ['Diferencias', `C$ ${data.totals.differences.toFixed(2)}`]], ['Caja', 'Cajero', 'Apertura', 'Estado', 'Facturas', 'Ventas', 'Efectivo', 'Esperado', 'Contado', 'Diferencia'], data.sessions.map((session) => [session.register, session.openedBy, new Date(session.openedAt).toLocaleString('es-NI'), session.status, session.invoiceCount, `C$ ${session.salesTotal.toFixed(2)}`, `C$ ${session.cashSales.toFixed(2)}`, `C$ ${session.expectedCash.toFixed(2)}`, session.countedCash == null ? '-' : `C$ ${session.countedCash.toFixed(2)}`, session.difference == null ? '-' : `C$ ${session.difference.toFixed(2)}`])); }
...
async function printThermalReceipt(invoice: SalesInvoice, business?: BusinessSettings) {
  ...
  printWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${receiptText(invoice.number)}</title><style>@page{size:${paperWidth}mm auto;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;width:${paperWidth}mm;background:#fff;color:#000}body{font:10px/1.35 "Courier New",monospace;padding:3mm}.center{text-align:center}.brand{font-size:18px;letter-spacing:1px}.line{border-top:1px dashed #000;margin:2mm 0}.row{display:flex;justify-content:space-between;gap:2mm;margin:.8mm 0}.info .row span:first-child{flex:0 0 16mm}.info .row span:last-child{text-align:right}.item{margin-bottom:2mm}.item b,.item small{display:block}.item small,.payments small{font-size:9px}.item div{display:flex;justify-content:space-between;margin-top:.5mm}.total{border-top:1px solid #000;border-bottom:1px solid #000;font-size:14px;padding:1mm 0;margin:1.5mm 0}.footer{font-weight:bold;padding:1mm 0 3mm;text-align:center}@media screen{body{margin:auto;border:1px solid #ddd;min-height:100vh}}</style></head><body><header class="center"><b class="brand">${receiptText(businessName)}</b>${business?.taxId ? `<div>${receiptText(business.taxId)}</div>` : ``}${business?.address ? `<div>${receiptText(business.address)}</div>` : ``}${business?.phone ? `<div>Tel. ${receiptText(business.phone)}</div>` : ``}<div>Comprobante de venta</div></header><div class="line"></div><section class="info"><div class="row"><span>Factura:</span><span>${receiptText(invoice.number)}</span></div><div class="row"><span>Fecha:</span><span>${receiptText(new Date(invoice.paidAt ?? invoice.createdAt).toLocaleString('es-NI'))}</span></div><div class="row"><span>Cliente:</span><span>${receiptText(invoice.customer?.name ?? 'Consumidor final')}</span></div><div class="row"><span>Cajero:</span><span>${receiptText(invoice.createdBy ? `${invoice.createdBy.firstName} ${invoice.createdBy.lastName}` : 'No disponible')}</span></div><div class="row"><span>Estado:</span><span>${receiptText(status)}</span></div></section><div class="line"></div>${itemRows}<div class="line"></div><div class="row"><span>Subtotal</span><span>C$ ${Number(invoice.subtotal).toFixed(2)}</span></div><div class="row"><span>Descuentos</span><span>- C$ ${Number(invoice.discountTotal).toFixed(2)}</span></div><div class="row"><span>Impuestos</span><span>C$ ${Number(invoice.taxTotal).toFixed(2)}</span></div><div class="row total"><b>TOTAL</b><b>C$ ${Number(invoice.total).toFixed(2)}</b></div><div class="row"><span>Pagado</span><span>C$ ${paid.toFixed(2)}</span></div><div class="row"><span>Cambio</span><span>C$ ${Number(invoice.changeAmount).toFixed(2)}</span></div><div class="line"></div><section class="payments"><div class="center"><b>FORMA DE PAGO</b></div>${paymentRows}</section><div class="line"></div><footer class="footer">${receiptText(business?.receiptMessage || `Gracias por su compra`)}</footer><script>window.addEventListener('load',()=>setTimeout(()=>window.print(),200));</script></body></html>`);
  printWindow.document.close();
  printWindow.focus();
}

function CashReportView({ user, onSalesReport, onProducts, onInventoryReport, onSales, onDashboard, onInventory, onCash, onAdmin, onLogout }: { user: AuthUser; onSalesReport: () => void; onProducts: () => void; onInventoryReport: () => void; onSales: () => Promise<void>; onDashboard: () => void; onInventory: () => Promise<void>; onCash: () => Promise<void>; onAdmin: () => Promise<void>; onLogout: () => void }) {
  ...
  function exportCash() { if (!data) return; const money = (value: number) => value.toFixed(2).replace('.', ','); downloadExcelCsv(`reporte-caja-${new Date().toISOString().slice(0, 10)}.csv`, ['Caja', 'Cajero', 'Apertura', 'Cierre', 'Estado', 'Facturas', 'Ventas totales', 'Ventas efectivo', 'Monto inicial', 'Esperado', 'Contado', 'Diferencia', 'Ingresos', 'Retiros', 'Gastos', 'Devoluciones'], data.sessions.map((session) => [session.register, session.openedBy, new Date(session.openedAt).toLocaleString('es-NI'), session.closedAt ? new Date(session.closedAt).toLocaleString('es-NI') : '', statusLabels[session.status] ?? session.status, session.invoiceCount, money(session.salesTotal), money(session.cashSales), money(session.openingAmount), money(session.expectedCash), session.countedCash == null ? '' : money(session.countedCash), session.difference == null ? '' : money(session.difference), money(session.income), money(session.withdrawals), money(session.expenses), money(session.returns)]), ['', '', '', '', 'TOTALES', '', money(data.totals.salesTotal), money(data.totals.cashSales), '', '', '', money(data.totals.differences), '', '', money(data.totals.expenses), '']); }
  return <main className="dashboard-shell"><aside className="sidebar"><div className="brand-lockup"><span className="brand-symbol">N</span><span>NexoPOS</span></div><p className="sidebar-caption">Centro de operación</p><nav className="main-nav"><button className="nav-item" onClick={onDashboard}><MenuIcon name="home" /><span>01</span>Resumen</button><button className="nav-item" onClick={() => { void onSales(); }}><MenuIcon name="sales" /><span>02</span>Ventas</button><button className="nav-item" onClick={() => { void onInventory(); }}><MenuIcon name="inventory" /><span>03</span>Inventario</button><button className="nav-item" onClick={() => { void onCash(); }}><MenuIcon name="cash" /><span>04</span>Caja</button><button className="nav-item active" onClick={onSalesReport}><MenuIcon name="reports" /><span>05</span>Reportes</button><button className="nav-item admin-nav" onClick={() => { void onAdmin(); }}><MenuIcon name="admin" /><span>06</span>Administración</button></nav><div className="sidebar-footer"><strong>{user.firstName} {user.lastName}</strong><button className="logout-button" onClick={onLogout}>Cerrar sesión</button></div></aside><section className="dashboard-content history-content"><header className="dashboard-header"><div><button className="back-action" onClick={onSalesReport}>← Volver a ventas</button><p className="eyebrow">Arqueos y movimientos</p><h1>Reporte de caja</h1><div className="report-export-actions"><button className="export-sales-button" type="button" disabled={!data?.sessions.length} onClick={exportCash}>↓ Excel</button><button className="export-sales-button" type="button" disabled={!data?.sessions.length} onClick={() => { if (data) printCashPdf(data, filters); }}>▣ PDF</button></div></div><div className="branch-selector"><span className="live-dot" />{user.branches[0]?.name}</div></header><div className="dashboard-rule" /><div className="report-tabs"><button onClick={onSalesReport}>Ventas</button><button onClick={onProducts}>Productos</button><button onClick={onInventoryReport}>Inventario</button><button className="active">Caja</button></div><section className="cash-report-filters"><label>Desde<input type="date" value={filters.dateFrom} onChange={(event) => setFilters((current) => ({ ...current, dateFrom: event.target.value }))} /></label><label>Hasta<input type="date" value={filters.dateTo} onChange={(event) => setFilters((current) => ({ ...current, dateTo: event.target.value }))} /></label><label>Estado<select value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value }))}><option value="">Todos</option><option value="OPEN">Abierta</option><option value="CLOSED">Cerrada</option><option value="WITH_DIFFERENCE">Con diferencia</option></select></label></section><div className="cash-report-metrics"><div><span>Turnos</span><strong>{data?.totals.sessions ?? 0}</strong></div><div><span>Ventas totales</span><strong>C$ {Number(data?.totals.salesTotal ?? 0).toFixed(2)}</strong></div><div><span>Ventas en efectivo</span><strong>C$ {Number(data?.totals.cashSales ?? 0).toFixed(2)}</strong></div><div><span>Diferencias</span><strong>C$ {Number(data?.totals.differences ?? 0).toFixed(2)}</strong></div></div><section className="cash-report-table"><div className="cash-report-row heading"><span>Caja / Cajero</span><span>Apertura</span><span>Estado</span><span>Ventas</span><span>Efectivo</span><span>Esperado</span><span>Contado</span><span>Diferencia</span><span>Gastos / Retiros</span></div>{loading ? <div className="catalog-empty"><span className="loading-spinner dark-spinner" /></div> : !data?.sessions.length ? <div className="catalog-empty"><p>No hay turnos con estos filtros.</p></div> : data.sessions.map((session) => <div className="cash-report-row" key={session.id}><span><strong>{session.register}</strong><small>{session.openedBy}</small></span><span>{new Date(session.openedAt).toLocaleString('es-NI')}<small>{session.closedAt ? `Cierre: ${new Date(session.closedAt).toLocaleString('es-NI')}` : ''}</small></span><em className={`cash-report-status status-${session.status.toLowerCase()}`}>{statusLabels[session.status] ?? session.status}</em><span>{session.invoiceCount}<small>C$ {session.salesTotal.toFixed(2)}</small></span><strong>C$ {session.cashSales.toFixed(2)}</strong><span>C$ {session.expectedCash.toFixed(2)}</span><span>{session.countedCash == null ? '—' : `C$ ${session.countedCash.toFixed(2)}`}</span><strong className={Number(session.difference ?? 0) === 0 ? 'positive-profit' : 'negative-profit'}>{session.difference == null ? '—' : `C$ ${session.difference.toFixed(2)}`}</strong><span>C$ {(session.expenses + session.withdrawals).toFixed(2)}<small>Ingresos C$ {session.income.toFixed(2)} · Dev. C$ {session.returns.toFixed(2)}</small></span></div>)}</section></section></main>;
}

function InventoryReportView({ user, onSalesReport, onProducts, onCashReport, onSales, onDashboard, onInventory, onCash, onAdmin, onLogout }: { user: AuthUser; onSalesReport: () => void; onProducts: () => void; onCashReport: () => void; onSales: () => Promise<void>; onDashboard: () => void; onInventory: () => Promise<void>; onCash: () => Promise<void>; onAdmin: () => Promise<void>; onLogout: () => void }) {
...
function ProductReportView({ user, onSales, onReports, onInventoryReport, onCashReport, onDashboard, onInventory, onCash, onAdmin, onLogout }: { user: AuthUser; onSales: () => Promise<void>; onReports: () => void; onInventoryReport: () => void; onCashReport: () => void; onDashboard: () => void; onInventory: () => Promise<void>; onCash: () => Promise<void>; onAdmin: () => Promise<void>; onLogout: () => void }) {
  ...
  function exportProducts() { if (!data) return; const number = (value: number, decimals = 2) => value.toFixed(decimals).replace('.', ','); downloadExcelCsv(`rentabilidad-productos-${new Date().toISOString().slice(0, 10)}.csv`, ['Código', 'Producto', 'Categoría', 'Vendidas', 'Devueltas', 'Unidades netas', 'Venta sin impuesto', 'Costo', 'Utilidad estimada', 'Margen %'], data.products.map((product) => [product.internalCode, product.name, product.category, number(product.soldQuantity, 3), number(product.returnedQuantity, 3), number(product.netQuantity, 3), number(product.netSales), number(product.cost), number(product.estimatedProfit), product.netSales > 0 ? number(product.estimatedProfit / product.netSales * 100) : number(0)]), ['', '', 'TOTALES', number(data.totals.soldQuantity, 3), number(data.totals.returnedQuantity, 3), number(data.totals.netQuantity, 3), number(data.totals.netSales), number(data.totals.cost), number(data.totals.estimatedProfit), number(margin)]); }
  return <main className="dashboard-shell"><aside className="sidebar"><div className="brand-lockup"><span className="brand-symbol">N</span><span>NexoPOS</span></div><p className="sidebar-caption">Centro de operación</p><nav className="main-nav"><button className="nav-item" type="button" onClick={onDashboard}><MenuIcon name="home" /><span>01</span>Resumen</button><button className="nav-item" type="button" onClick={() => { void onSales(); }}><MenuIcon name="sales" /><span>02</span>Ventas</button><button className="nav-item" type="button" onClick={() => { void onInventory(); }}><MenuIcon name="inventory" /><span>03</span>Inventario</button><button className="nav-item" type="button" onClick={() => { void onCash(); }}><MenuIcon name="cash" /><span>04</span>Caja</button><button className="nav-item active" type="button" onClick={onReports}><MenuIcon name="reports" /><span>05</span>Reportes</button><button className="nav-item admin-nav" type="button" onClick={() => { void onAdmin(); }}><MenuIcon name="admin" /><span>06</span>Administración</button></nav><div className="sidebar-footer"><strong>{user.firstName} {user.lastName}</strong><button className="logout-button" onClick={onLogout}>Cerrar sesión</button></div></aside><section className="dashboard-content history-content"><header className="dashboard-header"><div><button className="back-action" onClick={onReports}>← Volver a ventas</button><p className="eyebrow">Rentabilidad por producto</p><h1>Productos vendidos</h1><div className="report-export-actions"><button className="export-sales-button" type="button" disabled={!data?.products.length} onClick={exportProducts}>↓ Excel</button><button className="export-sales-button" type="button" disabled={!data?.products.length} onClick={() => { if (data) printProductsPdf(data, filters); }}>▣ PDF</button></div></div><div className="branch-selector"><span className="live-dot" />{user.branches[0]?.name}</div></header><div className="dashboard-rule" /><div className="report-tabs"><button type="button" onClick={onReports}>Ventas</button><button className="active" type="button">Productos</button><button type="button" onClick={onInventoryReport}>Inventario</button><button type="button" onClick={onCashReport}>Caja</button></div><section className="product-report-filters"><label>Buscar producto<input value={filters.search} onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} placeholder="Nombre, código o barras" /></label><label>Desde<input type="date" value={filters.dateFrom} onChange={(event) => setFilters((current) => ({ ...current, dateFrom: event.target.value }))} /></label><label>Hasta<input type="date" value={filters.dateTo} onChange={(event) => setFilters((current) => ({ ...current, dateTo: event.target.value }))} /></label></section><div className="product-report-metrics"><div><span>Unidades netas</span><strong>{Number(data?.totals.netQuantity ?? 0).toFixed(3)}</strong></div><div><span>Venta sin impuesto</span><strong>C$ {Number(data?.totals.netSales ?? 0).toFixed(2)}</strong></div><div><span>Costo estimado</span><strong>C$ {Number(data?.totals.cost ?? 0).toFixed(2)}</strong></div><div><span>Utilidad estimada</span><strong>C$ {Number(data?.totals.estimatedProfit ?? 0).toFixed(2)}</strong><small>{margin.toFixed(1)}% de margen</small></div></div><section className="product-report-table"><div className="product-report-row heading"><span>Producto</span><span>Categoría</span><span>Vendidas</span><span>Devueltas</span><span>Neto</span><span>Venta neta</span><span>Costo</span><span>Utilidad</span></div>{loading ? <div className="catalog-empty"><span className="loading-spinner dark-spinner" /></div> : !data?.products.length ? <div className="catalog-empty"><p>No hay productos vendidos con estos filtros.</p></div> : data.products.map((product) => <div className="product-report-row" key={product.productId}><span><strong>{product.name}</strong><small>{product.internalCode}</small></span><span>{product.category}</span><span>{product.soldQuantity.toFixed(3)}</span><span>{product.returnedQuantity.toFixed(3)}</span><strong>{product.netQuantity.toFixed(3)}</strong><span>C$ {product.netSales.toFixed(2)}</span><span>C$ {product.cost.toFixed(2)}</span><strong className={product.estimatedProfit < 0 ? 'negative-profit' : 'positive-profit'}>C$ {product.estimatedProfit.toFixed(2)}</strong></div>)}</section></section></main>;
}

function SalesHistoryView({ user, onProducts, onInventoryReport, onCashReport, onBack, onDashboard, onInventory, onCash, onAdmin, onLogout }: { user: AuthUser; onProducts: () => void; onInventoryReport: () => void; onCashReport: () => void; onBack: () => void; onDashboard: () => void; onInventory: () => Promise<void>; onCash: () => Promise<void>; onAdmin: () => Promise<void>; onLogout: () => void }) {
  ...
  async function exportSalesPdf() { const params = new URLSearchParams({ branchId: user.branches[0]?.id ?? '', pageSize: '100' }); Object.entries(filters).forEach(([key, value]) => { if (!['page', 'pageSize'].includes(key) && value) params.set(key, String(value)); }); const invoices: SalesInvoice[] = []; let page = 1; let pages = 1; let business = data?.business; do { params.set('page', String(page)); const response = await authenticatedFetch(`${apiUrl}/sales/history?${params}`); if (!response.ok) { await Swal.fire({ icon: 'error', title: 'No se pudo generar el PDF' }); return; } const result = await response.json() as SalesHistoryData; invoices.push(...result.items); business = result.business; pages = result.pagination.totalPages; page += 1; } while (page <= pages); const statusLabels: Record<string, string> = { PAID: 'Pagada', VOIDED: 'Anulada', PARTIALLY_RETURNED: 'Devuelta parcialmente', FULLY_RETURNED: 'Devuelta totalmente' }; const totals = invoices.reduce((sum, invoice) => ({ subtotal: sum.subtotal + Number(invoice.subtotal), discounts: sum.discounts + Number(invoice.discountTotal), taxes: sum.taxes + Number(invoice.taxTotal), total: sum.total + Number(invoice.total) }), { subtotal: 0, discounts: 0, taxes: 0, total: 0 }); const filterText = [filters.search && `Búsqueda: ${filters.search}`, filters.dateFrom && `Desde: ${filters.dateFrom}`, filters.dateTo && `Hasta: ${filters.dateTo}`, filters.status && `Estado: ${filters.status}`].filter(Boolean).join(' | '); printReportPdf('Reporte de ventas', business, filterText, [['Subtotal', `C$ ${totals.subtotal.toFixed(2)}`], ['Descuentos', `C$ ${totals.discounts.toFixed(2)}`], ['Impuestos', `C$ ${totals.taxes.toFixed(2)}`], ['Total', `C$ ${totals.total.toFixed(2)}`]], ['Factura', 'Fecha', 'Cliente', 'Cajero', 'Estado', 'Subtotal', 'Descuentos', 'Impuestos', 'Total'], invoices.map((invoice) => [invoice.number, new Date(invoice.createdAt).toLocaleString('es-NI'), invoice.customer?.name ?? 'Consumidor final', invoice.createdBy ? `${invoice.createdBy.firstName} ${invoice.createdBy.lastName}` : '', statusLabels[invoice.status] ?? invoice.status, `C$ ${Number(invoice.subtotal).toFixed(2)}`, `C$ ${Number(invoice.discountTotal).toFixed(2)}`, `C$ ${Number(invoice.taxTotal).toFixed(2)}`, `C$ ${Number(invoice.total).toFixed(2)}`])); }
  async function exportSales() {
    ...
    try {
      ...
      const rows = invoices.map((invoice) => [invoice.number, new Date(invoice.createdAt).toLocaleString('es-NI'), invoice.customer?.name ?? 'Consumidor final', invoice.createdBy ? `${invoice.createdBy.firstName} ${invoice.createdBy.lastName}` : '', statusLabels[invoice.status] ?? invoice.status, amount(invoice.subtotal), amount(invoice.discountTotal), amount(invoice.taxTotal), amount(invoice.total), invoice.payments.map((payment) => payment.paymentMethod.name).join(' + ')].map(quote).join(';'));
      const header = ['Factura', 'Fecha', 'Cliente', 'Cajero', 'Estado', 'Subtotal', 'Descuentos', 'Impuestos', 'Total', 'Métodos de pago'].map(quote).join(';');
      const totals = invoices.reduce((summary, invoice) => ({ subtotal: summary.subtotal + Number(invoice.subtotal), discounts: summary.discounts + Number(invoice.discountTotal), taxes: summary.taxes + Number(invoice.taxTotal), total: summary.total + Number(invoice.total) }), { subtotal: 0, discounts: 0, taxes: 0, total: 0 });
      const totalRow = ['', '', '', '', 'TOTALES', amount(totals.subtotal), amount(totals.discounts), amount(totals.taxes), amount(totals.total), ''].map(quote).join(';');
