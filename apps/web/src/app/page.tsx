...
export default function HomePage() {
  ...
  if (user) {
    ...
    if (activeView === 'sales-history') return <SalesHistoryView user={user} onProducts={() => setActiveView('product-report')} onInventoryReport={() => setActiveView('inventory-report')} onBack={() => setActiveView('sales')} onDashboard={() => setActiveView('dashboard')} onInventory={openCatalog} onCash={openCash} onAdmin={openAdmin} onLogout={handleLogout} />;
    if (activeView === 'product-report') return <ProductReportView user={user} onSales={openSales} onReports={() => setActiveView('sales-history')} onInventoryReport={() => setActiveView('inventory-report')} onDashboard={() => setActiveView('dashboard')} onInventory={openCatalog} onCash={openCash} onAdmin={openAdmin} onLogout={handleLogout} />;
    if (activeView === 'inventory-report') return <InventoryReportView user={user} onSalesReport={() => setActiveView('sales-history')} onProducts={() => setActiveView('product-report')} onSales={openSales} onDashboard={() => setActiveView('dashboard')} onInventory={openCatalog} onCash={openCash} onAdmin={openAdmin} onLogout={handleLogout} />;
...
function InventoryReportView({ user, onSalesReport, onProducts, onSales, onDashboard, onInventory, onCash, onAdmin, onLogout }: { user: AuthUser; onSalesReport: () => void; onProducts: () => void; onSales: () => Promise<void>; onDashboard: () => void; onInventory: () => Promise<void>; onCash: () => Promise<void>; onAdmin: () => Promise<void>; onLogout: () => void }) {
