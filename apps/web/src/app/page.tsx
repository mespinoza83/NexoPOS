...
export default function HomePage() {
  ...
  if (user) {
    ...
    if (activeView === 'sales' && salesSetup) return <SalesView user={user} products={products} setup={salesSetup} invoices={salesHistory} loading={salesLoading} onRefresh={openSales} onHistory={() => setActiveView('sales-history')} onBack={() => setActiveView('dashboard')} onInventory={openCatalog} onCash={openCash} onAdmin={openAdmin} onLogout={handleLogout} />;
    if (activeView === 'sales-history') return <SalesHistoryView user={user} onBack={() => setActiveView('sales')} onDashboard={() => setActiveView('dashboard')} onLogout={handleLogout} />;
...
function SalesHistoryView({ user, onBack, onDashboard, onLogout }: { user: AuthUser; onBack: () => void; onDashboard: () => void; onLogout: () => void }) {
  ...
  async function printInvoice(invoice: SalesInvoice) { const response = await authenticatedFetch(`${apiUrl}/sales/${invoice.id}/reprint`, { method: 'POST' }); if (response.ok) window.print(); else await Swal.fire({ icon: 'error', title: 'No se puede imprimir', text: 'Verifica el permiso de reimpresión.' }); }
...
function SalesView({ user, products, setup, invoices, loading, onRefresh, onHistory, onBack, onInventory, onCash, onAdmin, onLogout }: { user: AuthUser; products: CatalogProduct[]; setup: SalesSetup; invoices: SalesInvoice[]; loading: boolean; onRefresh: () => Promise<void>; onHistory: () => void; onBack: () => void; onInventory: () => Promise<void>; onCash: () => Promise<void>; onAdmin: () => Promise<void>; onLogout: () => void }) {
