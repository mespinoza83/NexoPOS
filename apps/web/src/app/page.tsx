...
async function printThermalReceipt(invoice: SalesInvoice, business?: BusinessSettings) {
  ...
  const itemRows = invoice.items.map((item) => `<div class="item"><b>${receiptText(item.product.name)}</b><small>${receiptText(item.product.internalCode)}</small><div><span>${Number(item.quantity).toFixed(3)} x ${currencySymbol} ${Number(item.unitPrice).toFixed(2)}</span><strong>${currencySymbol} ${Number(item.lineTotal).toFixed(2)}</strong></div></div>`).join('');
