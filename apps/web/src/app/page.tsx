...
export default function HomePage() {
  ...
  if (user) {
    ...
    if (activeView === 'sales-history') return <SalesHistoryView user={user} onProducts={() => setActiveView('product-report')} onBack={() => setActiveView('sales')} onDashboard={() => setActiveView('dashboard')} onInventory={openCatalog} onCash={openCash} onAdmin={openAdmin} onLogout={handleLogout} />;
    if (activeView === 'product-report') return <ProductReportView user={user} onSales={openSales} onReports={() => setActiveView('sales-history')} onDashboard={() => setActiveView('dashboard')} onInventory={openCatalog} onCash={openCash} onAdmin={openAdmin} onLogout={handleLogout} />;
...
function ProductReportView({ user, onSales, onReports, onDashboard, onInventory, onCash, onAdmin, onLogout }: { user: AuthUser; onSales: () => Promise<void>; onReports: () => void; onDashboard: () => void; onInventory: () => Promise<void>; onCash: () => Promise<void>; onAdmin: () => Promise<void>; onLogout: () => void }) {
