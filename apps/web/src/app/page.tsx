...
function formatMeasuredQuantity(value: string | number, unit: UnitOfMeasure) { return `${Number(value).toFixed(2)} ${unitLabel(unit)}`; }
...
async function printThermalReceipt(invoice: SalesInvoice, business?: BusinessSettings) {
  ...
  const itemRows = invoice.items.map((item) => `<div class="item"><b>${receiptText(item.product.name)}</b><small>${receiptText(item.product.internalCode)}</small><div><span>${Number(item.quantity).toFixed(2)} ${receiptText(unitLabel(item.saleUnit))} x ${currencySymbol} ${Number(item.unitPrice).toFixed(2)}</span><strong>${currencySymbol} ${Number(item.lineTotal).toFixed(2)}</strong></div></div>`).join('');
      ...
      <section className={`dashboard-content catalog-content inventory-tab-${inventoryTab}`}>
        ...
        {showProductForm && <form className="product-form" onSubmit={saveProduct}>
          ...
          <div className="product-form-grid">
            ...
            {!editingProduct && <label>Existencia inicial<input name="initialQuantity" type="number" min="0" step="0.01" required defaultValue="0" /></label>}
            <label>Existencia mínima<input name="minimumQuantity" type="number" min="0" step="0.01" required defaultValue={editingProduct?.inventory[0]?.minimumQuantity ?? 0} /></label>
